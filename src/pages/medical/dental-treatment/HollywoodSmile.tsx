  ...
  <div className="bg-[#f7f9fc] min-h-screen pb-12">
    ...
    <div className="relative h-64 md:h-80 w-full flex items-center justify-center mb-8" style={{ backgroundImage: 'url(/images/hero-placeholder.png)', backgroundSize: 'cover', backgroundPosition: 'center' }}>
      ...
      <div className="relative z-10 text-center text-white w-full">
        <h1 className="text-4xl md:text-5xl font-extrabold uppercase tracking-widest">HOLLYWOOD SMILE</h1>
      </div>
    </div>
    <div className="max-w-7xl mx-auto flex flex-col md:flex-row gap-8 px-4">
      {/* Left: Main Content */}
      <div className="flex-1">
        <h2 className="text-2xl font-bold mb-2">Complete Hollywood Smile Makeover</h2>
        <p className="text-gray-700 mb-4">A Hollywood Smile makeover combines advanced cosmetic dental treatments to create a flawless, radiant smile. Our expert team in Turkey uses the latest techniques and materials to deliver natural, beautiful, and long-lasting results tailored to your unique features.</p>
        ...
        <p className="text-gray-700 mb-4">Our Hollywood Smile service includes a comprehensive approach to smile design, ensuring natural results and patient satisfaction:</p>
        <ol className="list-decimal pl-6 text-gray-700 space-y-1 mb-8">
          <li>Personalized smile design and consultation.</li>
          <li>Porcelain veneers, crowns, and teeth whitening.</li>
          <li>Minimally invasive procedures for a natural look.</li>
          <li>Expert cosmetic dentists and advanced technology.</li>
        ...
        <div className="bg-white rounded-xl shadow p-6 mb-4">
          ...
          <ul className="space-y-2">
            {dentalTreatmentServices.map((service) => (
              <li key={service.link}>
                <a href={service.link} className={`text-blue-700 hover:underline ${service.name === 'Hollywood Smile' ? 'font-semibold' : ''}`}>{service.name}</a>
              </li>
            ))}
          </ul>
        </div>
